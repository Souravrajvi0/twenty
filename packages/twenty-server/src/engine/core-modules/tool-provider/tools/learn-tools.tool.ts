...
export const createLearnToolsTool = (
  ...
  execute: async (parameters: LearnToolsInput): Promise<LearnToolsResult> => {
    ...
    // Only the tool schemas are bulky; message, notFound and suggestions must
    // stay inline so the model can react without paging the spilled file.
    ...
    // The spill service returns the candidate untouched when it fits inline.
